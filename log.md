...
- 정리: 기존 baseline은 유지한 채 `네 층` 오기를 `다섯 층`으로 바로잡고, 후반 자기서사 구간을 3역할 표로 다시 압축해 긴 changelog가 `누가 어떤 종류의 유지 노동을 맡고 있었는가`까지 더 빨리 읽히도록 다듬었다.
- 선호 신호: 저자는 CHANGELOG에서도 사건 나열보다, 같은 목록 안에서 발행 판단·기반 상속·생활형 손질이 어떤 역할 구조로 겹치는지가 보이는 정리를 더 선호하는 경향이 강하다.
